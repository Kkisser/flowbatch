...
_ADVICE = {
    ...
    ERR_STALE_PICKER: (
        "Flow не показал нужный элемент (референс, картинку @lib или проект в "
        "пикере) даже после перезагрузки вкладки. Проверь, что он существует "
        "именно там, где его ищут: задача-источник отработала в ЭТОМ проекте, "
        "имя @lib и имя проекта написаны точно."
    ),
}
