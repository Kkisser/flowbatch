...
2. Товар Ревилайн под сюжет (если не назван — бери флагман RL 230).
3. Имя продукта в базе программы (подпапка `products/`, напр. `rl230`).
...
Директивы (других не существует):
  ...
  @product <имя>     — фото товара из базы: `@product rl230` прикрепляет ВСЕ фото
                       из папки продукта. Именно так и пиши — всей папкой, без
                       указания конкретных файлов: разные ракурсы работают на
...
- Фото товара — @product в КАЖДОМ блоке (и IMG, и VID), где товар в кадре
  ИЛИ ВООБЩЕ МОЖЕТ в нём оказаться: держит в руках, лежит рядом, виден на
  фоне, упоминается в действии. С персонажами лишний референс вредит, с
  товаром — наоборот: без референса модель нарисует выдуманный прибор.
  Сомневаешься, попадёт ли товар в кадр, — прикрепляй.
...
@product rl230
# Сцена 2 — Image. В кадре: сын с ирригатором у раковины.
3D Pixar style, vertical 9:16. ...
...
=== ПРОДУКТ ===
Товар подбираем под сцену: щётки, пасты, ирригаторы, флоссы, ополаскиватели, ёршики. По умолчанию флагман — портативный ирригатор Ревилайн RL 230, чёрный (REVYLINE RL 230, папка продукта `rl230`).
...
Эталон внешнего вида RL 230 для промптов (вставлять дословно в сцены с ним; описание сверено с реальными фото продукта):
```
The device is the Revyline RL 230 portable oral irrigator: a slim upright bottle-shaped handheld body, the upper half glossy black, the lower half a translucent smoky-grey water reservoir that tapers gently toward the base, a thin slightly curved black nozzle rising from the top, small uppercase "REVYLINE" lettering near the top of the black body, a narrow front control strip with two small square buttons (power on top, "M" below) and a vertical column of four tiny mode indicator dots labeled Normal, Soft, Pulse and DIY, a small charging-port flap on the lower reservoir. Its exact colors, proportions, finish, buttons, nozzle and lettering match the attached reference photos exactly. Do not change its color or shape, do not add extra buttons, displays, hoses or logos, only one device in frame.
```

Характеристики RL 230 для реплик: портативный, резервуар 300 мл, 4 режима (Normal, Soft, Pulse и DIY — в нём 10 уровней давления на выбор), давление струи до 900 кПа, 1200–1600 пульсаций в минуту, аккумулятор 1500 мА·ч, влагозащита IPX7 — можно брать в душ и в дорогу.
...
7. У каждого IMG-блока сцен 2+ есть @use на эталон. @product (всей папкой, без имён файлов) стоит в КАЖДОМ блоке — и IMG, и VID, — где товар в кадре или может в нём оказаться. Ни в одном блоке нет референса ПЕРСОНАЖА, которого нет в кадре.
